...
class PNChartViewController: UIViewController {
    ...
    @IBAction func buttonpressed() {
        ...
        let item3 = PNPieChartDataItem(value: 70, color: UIColor.redColor(), description: "Diff3")
    ...
    override func viewDidLoad() {
        ...
        let item3 = PNPieChartDataItem(value: 30, color: UIColor.redColor(), description: "Stat3")
        let items = [item1, item2, item3]
        pieChart = PNPieChart(frame: CGRect(x: 50, y: self.view.center.y-200, width: 300, height: 300), items: items)
        pieChart!.strokeChart()
